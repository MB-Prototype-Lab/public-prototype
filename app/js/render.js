...
function adminSubtitle() {
  ...
  if (state.screen === "spendingProfile") return APP_VERSION + " step 1 — twelve sliders, opening on ZIP-adjusted national averages.";
  if (state.screen === "budgetCompare")   return APP_VERSION + " step 3 — per category, their figure vs the model's vs the midpoint.";
...
function render() {
  ...
  if (scrollKey() === lastPaintedScreen) {
    ...
    scrollTop();
    // The view actually changed, so the address bar should say so. This branch
    // is the app's only agreed definition of "a new page to a tester", which is
    // exactly the unit a usability tracker records (js/screen-url.js).
    if (typeof screenUrlSync === "function") screenUrlSync();
  }
  lastPaintedScreen = scrollKey();

  // Name every control the tester can reach, so a click report says what was
  // pressed rather than where (js/ub-names.js). Unconditional, not inside the
  // branch above: a repaint that holds the view still replaces the markup, and
  // the new nodes carry no names until they are stamped.
  if (typeof ubStampControls === "function") ubStampControls();

  // Update admin footer — nav log and last error (skip when admin is collapsed)
  if (!state.adminCollapsed) {
    const trailEl = document.getElementById('adminTrailLog');
    if (trailEl && typeof ubTrailPreview === "function") {
      const t = ubTrailPreview();
      trailEl.textContent = 'Trail: ' + (t.indexOf('.') === -1 ? 'nothing set yet' : t.slice(8));
    }
